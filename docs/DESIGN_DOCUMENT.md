...
**煮理人** 是一款专为同居情侣或小家庭设计的微信小程序，旨在帮助家庭成员管理厨房相关的日常事务。
...
1. **前端请求：** 页面通过 `wx.cloud.callFunction()` 发起云函数调用
2. **云函数处理：** 云函数接收参数，通过 `wx-server-sdk` 访问数据库
3. **数据操作：** 云函数执行增删改查操作
4. **统一响应：** 返回格式为 `{success: boolean, data?: any, message?: string}`
5. **前端更新：** 页面接收响应后更新视图状态
...
```javascript
{
  ...
  description: "任务描述",
  rewardScore: 10,              // 奖励积分
  status: "pending" | "doing" | "review" | "done" | "cancel",
  creatorId: "创建者ID",
...
- `familyId` - 家庭签到
- `date` - 日期查询
- `userId + date` (复合索引) - 单日签到检查

---

### 3.10 shop_items - 商城商品表
...
| 页面 | 路径 | 功能 |
|------|------|------|
| 首页 | pages/home/home | 家庭概览、推荐、快捷入口 |
| 登录 | pages/login/login | 用户登录与授权 |
| 家庭 | pages/family/family | 创建或加入家庭 |
| 任务 | pages/task/task | 任务列表与管理 |
| 创建任务 | pages/task-create/task-create | 创建新任务 |
| 菜谱 | pages/recipe/recipe | 菜谱列表与搜索 |
| 菜谱详情 | pages/recipe-detail/recipe-detail | 菜谱详情，开始烹饪 |
| 创建菜谱 | pages/recipe-create/recipe-create | 创建/编辑菜谱 |
| 冰箱 | pages/fridge/fridge | 冰箱库存管理 |
| 互动 | pages/interaction/interaction | 签到历史、夸夸 |
| 排名 | pages/rank/rank | 积分排行榜 |
| 商城 | pages/shop/shop | 积分商城 |
...
5. 调用 getOpenId 云函数
   ↓
6. 获取 openId，创建/更新用户记录
   ↓
7. 存储 openId 和 userInfo 到全局和本地缓存
   ↓
8. 检查是否已加入家庭
   ↓ (否)
9. 跳转到家庭页
...
2. 输入家庭名称
   ↓
3. 调用 familyManager/create
   ↓
4. 生成6位邀请码
   ↓
5. 创建家庭记录，设置创建者为 owner
   ↓
6. 更新用户 familyId
   ↓
7. 显示邀请码，引导邀请成员
...
review (待确认)
   ↓ [创建者批准/驳回]
done (已完成) / doing (退回重新做)

任何状态 → cancel (已取消)
```

**积分发放：**
- 任务被批准（done）时，自动给执行人发放积分
- 同时记录到 score_logs
